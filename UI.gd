...
func _RefreshProgress():
	...
	if curLevel < GData.user.level:
		curLevel = GData.user.level
		tween.tween_property(ExpBar, "value", ExpBar.max_value, 1).set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_IN_OUT)
		await tween.finished
		tween = get_tree().create_tween()
		ExpBar.set_value(0)
	Level.set_text("Lv. " + str(GData.user.level))
	ExpValue.set_text(str(GData.user.exp) + "/" + str(GData.user.expNeededForNextLevel))
	tween.tween_property(ExpBar, "value", GData.user.exp, 1).set_trans(Tween.TRANS_CUBIC).set_ease(Tween.EASE_IN_OUT)
